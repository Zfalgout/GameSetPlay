...
def open
	...
	@doublesMatches = Match.where("open = ? AND game_type = ? OR player2 = ? OR player3 = ? OR player4 = ?", 1, "Doubles", 'NOPLAYER', 'NOPLAYER', 'NOPLAYER').where.not(player1: current_user.id, player2: current_user.id, player3: current_user.id, player4: current_user.id).where(time: Date.today..3.years.from_now).all.paginate(page: params[:page])
	#@matches = Match.paginate(page: params[:page])
end
