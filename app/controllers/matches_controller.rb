...
def open
	...
	if (params[:search1] && params[:search2])
		...
		@matches = Match.search(params[:search1], params[:search2]).where("open = ? AND player2 = ? OR player3 = ? OR player4 = ?", 1, 'Player 2', 'Player 3', 'Player 4').where.not(player1: current_user.id, player2: current_user.id, player3: current_user.id, player4: current_user.id).where(time: Date.today..3.years.from_now).all.paginate(page: params[:page])
		flash[:info] = "#{@search1} #{@search2}"
